...
dat$treat <- dat$id
dat$treat <- mapvalues(dat$treat, from = c('2', '6', '14', '19'), to = c(rep('TS-Bo', 4)))
dat$treat <- mapvalues(dat$treat, from = c('4', '7', '11', '17'), to = c(rep('TH', 4)))
dat$treat <- mapvalues(dat$treat, from = c('3', '8', '12', '15'), to = c(rep('TS-TSB', 4)))
dat$treat <- mapvalues(dat$treat, from = c('1', '10', '13', '18'), to = c(rep('TS-TSB+', 4)))
...
dat$exp <- '24J'                # CHANGE CHANGE CHANGE
...
write.xlsx(dat.table, file = '../ALFAM2 submission/dat_24J.xlsx')