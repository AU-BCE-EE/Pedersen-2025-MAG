...
dat$treat <- dat$id
dat$treat <- mapvalues(dat$treat, from = c('1', '10', '15'), to = c(rep('A', 3)))
dat$treat <- mapvalues(dat$treat, from = c('2', '7', '13'), to = c(rep('Cattle B', 3)))
dat$treat <- mapvalues(dat$treat, from = c('3', '8', '19'), to = c(rep('Pig B', 3)))
dat$treat <- mapvalues(dat$treat, from = c('6', '12', '18'), to = c(rep('G', 3)))
dat$treat <- mapvalues(dat$treat, from = c('4', '11', '14'), to = c(rep('C', 3)))

dat$replicate <- dat$id
dat$replicate <- mapvalues(dat$replicate, from = c('1', '2', '3', '4', '6'), to = rep('1', 5))
dat$replicate <- mapvalues(dat$replicate, from = c('7', '8', '10', '11', '12'), to = rep('2', 5))
dat$replicate <- mapvalues(dat$replicate, from = c('13', '14', '15', '19', '18'), to = rep('3', 5))
...
dat$plot.num <- mapvalues(dat$plot.num, from = c('16'), to = c('14'))
...
# time got messed up in the first round: 
dat[dat$id == 18, ]$elapsed.time <- dat[dat$id == 1, ]$elapsed.time[1:66]
dat[dat$id == 19, ]$elapsed.time <- dat[dat$id == 1, ]$elapsed.time[1:66]
...
dat$exp <- '24F'                # CHANGE CHANGE CHANGE
...
write.xlsx(dat.table, file = '../ALFAM2 submission/dat_24F.xlsx')