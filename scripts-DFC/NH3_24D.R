...
dat$treat <- dat$id
dat$treat <- mapvalues(dat$treat, from = c('3', '7', '12', '15'), to = c(rep('TS-Bo', 4)))
dat$treat <- mapvalues(dat$treat, from = c('4', '8', '11', '17'), to = c(rep('TS-TSB+', 4)))
dat$treat <- mapvalues(dat$treat, from = c('1', '6', '13', '18'), to = c(rep('TH', 4)))
dat$treat <- mapvalues(dat$treat, from = c('2', '10', '14', '19'), to = c(rep('TS-TSB', 4)))
...
# removing four elapsed.times. Due to a field trip with visitors the valvebox was open.
# it is assumed that the negative emissions in this period is due to the valve-section 
# of the setup being cold and the recovery time was too slow. 

dat <- dat[! dat$elapsed.time == 27.90, ]
dat <- dat[! dat$elapsed.time == 30.40, ]
dat <- dat[! dat$elapsed.time == 32.90, ]
dat <- dat[! dat$elapsed.time == 35.40, ]

# removing valuve 3 + 17 as they seem like they are disconnected (negative values)
# will be cheked in the field! 
dat <- dat[! dat$id == '3', ]
dat <- dat[! dat$id == '17', ]
...
dat$exp <- '24D'                # CHANGE CHANGE CHANGE
...
write.xlsx(dat.table, file = '../ALFAM2 submission/dat_24D.xlsx')