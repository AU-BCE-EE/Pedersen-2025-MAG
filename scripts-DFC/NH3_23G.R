...
dat$treat <- dat$id
dat$treat <- mapvalues(dat$treat, from = c('2', '10', '14'), to = c(rep('B', 3)))
dat$treat <- mapvalues(dat$treat, from = c('6', '8', '17'), to = c(rep('B sep', 3)))
dat$treat <- mapvalues(dat$treat, from = c('4', '7', '13'), to = c(rep('C', 3)))
dat$treat <- mapvalues(dat$treat, from = c('1', '11', '15'), to = c(rep('A', 3)))
dat$treat <- mapvalues(dat$treat, from = c('3', '12', '18'), to = c(rep('C sep', 3)))
...
# write.xlsx(dat.table, file = '../ALFAM2 submission/dat_22G.xlsx')
