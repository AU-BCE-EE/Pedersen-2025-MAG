...
dat$treat <- dat$id
dat$treat <- mapvalues(dat$treat, from = c('1', '8', '14'), to = c(rep('A dec', 3)))
dat$treat <- mapvalues(dat$treat, from = c('2', '10', '15'), to = c(rep('A MF slurry', 3)))
dat$treat <- mapvalues(dat$treat, from = c('7', '13', '17'), to = c(rep('A MF liquid', 3)))
dat$treat <- mapvalues(dat$treat, from = c('4', '6', '12', '19'), to = c(rep('A screw', 4)))
dat$treat <- mapvalues(dat$treat, from = c('3', '11', '18'), to = c(rep('A', 3)))

dat$replicate <- dat$id
dat$replicate <- mapvalues(dat$replicate, from = c('1', '2', '3', '4', '7'), to = rep('1', 5))
dat$replicate <- mapvalues(dat$replicate, from = c('6', '8', '10', '11', '13'), to = rep('2', 5))
dat$replicate <- mapvalues(dat$replicate, from = c('12', '14', '15', '17', '18'), to = rep('3', 5))
dat$replicate <- mapvalues(dat$replicate, from = c('19'), to = rep('4', 1))
...
dat$exp <- '24M'                # CHANGE CHANGE CHANGE
...
write.xlsx(dat.table, file = '../ALFAM2 submission/dat_24M.xlsx')