...
dat$treat <- dat$id
dat$treat <- mapvalues(dat$treat, from = c('1', '6', '11', '15'), to = c(rep('TS-Bo', 4)))
dat$treat <- mapvalues(dat$treat, from = c('4', '10', '14', '19'), to = c(rep('TS-TSB+', 4)))
dat$treat <- mapvalues(dat$treat, from = c('2', '7', '12', '17'), to = c(rep('TH', 4)))
dat$treat <- mapvalues(dat$treat, from = c('3', '8', '13', '18'), to = c(rep('TS-TSB', 4)))
...
dat$exp <- '24C'                # CHANGE CHANGE CHANGE
...
write.xlsx(dat.table, file = '../ALFAM2 submission/dat_24C.xlsx')