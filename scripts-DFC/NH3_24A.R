...
dat$treat <- dat$id
dat$treat <- mapvalues(dat$treat, from = c('1', '11', '14'), to = c(rep('Un30', 3)))
dat$treat <- mapvalues(dat$treat, from = c('4', '12', '15'), to = c(rep('Un25', 3)))
dat$treat <- mapvalues(dat$treat, from = c('2', '7', '18'), to = c(rep('Un12', 3)))
dat$treat <- mapvalues(dat$treat, from = c('6', '17', '19'), to = c(rep('Sep30', 3)))
dat$treat <- mapvalues(dat$treat, from = c('3', '8', '13'), to = c(rep('Sep25', 3)))

dat$replicate <- dat$id
dat$replicate <- mapvalues(dat$replicate, from = c('1', '4', '2', '3', '6'), to = rep('1', 5))
dat$replicate <- mapvalues(dat$replicate, from = c('11', '12', '7', '17', '8'), to = rep('2', 5))
dat$replicate <- mapvalues(dat$replicate, from = c('14', '15', '18', '19', '13'), to = rep('3', 5))
...
# removing valve 10 (DFC 8) as it is out of order
dat <- dat[! dat$id == 10, ]

# time got messed up for id 18: 
dat[dat$id == 18, ]$elapsed.time <- dat[dat$id == 1, ]$elapsed.time[1:152]
...
# flux is g-N * min*-1 * m^-2 evaporated. 
# L slurry applied (L * m^-2): UN30: 1.66 L; UN25: 1.38 L; Un12: 0.70 L
# TAN in the slurry: 1.69 g * kg^-1 for untreated and 1.21 g * kg^-1 for separated
# assuming density of 1
dat[dat$treat == 'Un30', ]$flux <- dat[dat$treat == 'Un30', ]$flux / (1.66*1.69)
dat[dat$treat == 'Un25', ]$flux <- dat[dat$treat == 'Un25', ]$flux / (1.38*1.69)
dat[dat$treat == 'Un12', ]$flux <- dat[dat$treat == 'Un12', ]$flux / (0.7*1.69)
dat[dat$treat == 'Sep30', ]$flux <- dat[dat$treat == 'Sep30', ]$flux / (1.66*1.21)
dat[dat$treat == 'Sep25', ]$flux <- dat[dat$treat == 'Sep25', ]$flux / (1.38*1.21)
...
dat <- dat[! c(dat$elapsed.time == 337 & dat$id == 2), ]
dat <- dat[! c(dat$elapsed.time == 339 & dat$id == 2), ]

############# MAKING DATA FRAME FOR ALFAM2 TEMPLATE 

dat$project <- 'MAG'             # CHANGE CHANGE CHANGE
dat$exp <- '24A'                # CHANGE CHANGE CHANGE
...
# write.xlsx(dat.table, file = '../ALFAM2 submission/dat_24A.xlsx')
