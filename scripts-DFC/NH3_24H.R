...
dat$treat <- dat$id
dat$treat <- mapvalues(dat$treat, from = c('4', '8', '13', '17'), to = c(rep('TH-12', 4)))
dat$treat <- mapvalues(dat$treat, from = c('2', '10', '12', '15'), to = c(rep('TH-4', 4)))
dat$treat <- mapvalues(dat$treat, from = c('3', '7', '11', '19'), to = c(rep('TS-TSB-12', 4)))
dat$treat <- mapvalues(dat$treat, from = c('1', '6', '14', '18'), to = c(rep('TS-TSB-4', 4)))
...
dat$exp <- '24H'                # CHANGE CHANGE CHANGE
...
write.xlsx(dat.table, file = '../ALFAM2 submission/dat_24H.xlsx')