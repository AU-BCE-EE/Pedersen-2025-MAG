...
dat$treat <- dat$id
dat$treat <- mapvalues(dat$treat, from = c('3', '10', '13', '17'), to = c(rep('TS-TSB', 4)))
dat$treat <- mapvalues(dat$treat, from = c('2', '6', '11', '19'), to = c(rep('TS-TSB+', 4)))
dat$treat <- mapvalues(dat$treat, from = c('4', '8', '14', '18'), to = c(rep('TS-Bo', 4)))
dat$treat <- mapvalues(dat$treat, from = c('1', '7', '12', '15'), to = c(rep('TH', 4)))
...
dat$exp <- '24L'                # CHANGE CHANGE CHANGE
...
write.xlsx(dat.table, file = '../ALFAM2 submission/dat_24L.xlsx')