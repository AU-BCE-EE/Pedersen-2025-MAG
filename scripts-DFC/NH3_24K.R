...
dat$treat <- dat$id
dat$treat <- mapvalues(dat$treat, from = c('3', '7', '11', '13', '18'), to = c(rep('No', 5)))
dat$treat <- mapvalues(dat$treat, from = c('1', '6', '8', '12', '15'), to = c(rep('2-pos', 5)))
dat$treat <- mapvalues(dat$treat, from = c('2', '4', '10', '14', '17'), to = c(rep('7-pos', 5)))
dat$treat <- mapvalues(dat$treat, from = c('19'), to = c(rep('Bare soil', 1)))

dat$replicate <- dat$id
dat$replicate <- mapvalues(dat$replicate, from = c('1', '2', '3'), to = rep('1', 3))
dat$replicate <- mapvalues(dat$replicate, from = c('4', '6', '7'), to = rep('2', 3))
dat$replicate <- mapvalues(dat$replicate, from = c('8', '10', '11'), to = rep('3', 3))
dat$replicate <- mapvalues(dat$replicate, from = c('12', '13', '14'), to = rep('4', 3))
dat$replicate <- mapvalues(dat$replicate, from = c('15', '17', '18'), to = rep('5', 3))
...
# removing a data error: 
dat <- dat[! dat$elapsed.time == 3.73, ]
...
dat$exp <- '24K'                # CHANGE CHANGE CHANGE
...
write.xlsx(dat.table, file = '../ALFAM2 submission/dat_24K.xlsx')