...
dat$treat <- dat$id
dat$treat <- mapvalues(dat$treat, from = c('1', '11', '17'), to = c(rep('B', 3)))
dat$treat <- mapvalues(dat$treat, from = c('2', '10', '18'), to = c(rep('A acid', 3)))
dat$treat <- mapvalues(dat$treat, from = c('3', '12', '15'), to = c(rep('B sep acid', 3)))
dat$treat <- mapvalues(dat$treat, from = c('4', '7', '14'), to = c(rep('A', 3)))
dat$treat <- mapvalues(dat$treat, from = c('6', '8', '13'), to = c(rep('B acid', 3)))
...
# write.xlsx(dat.table, file = '../ALFAM2 submission/dat_22D.xlsx')
