...
dat$treat <- dat$id
dat$treat <- mapvalues(dat$treat, from = c('3', '8', '13'), to = c(rep('A', 3)))
dat$treat <- mapvalues(dat$treat, from = c('6', '10', '14'), to = c(rep('Cattle C', 3)))
dat$treat <- mapvalues(dat$treat, from = c('2', '11', '18'), to = c(rep('Pig C', 3)))
dat$treat <- mapvalues(dat$treat, from = c('4', '12', '19'), to = c(rep('B', 3)))
dat$treat <- mapvalues(dat$treat, from = c('1', '7', '15'), to = c(rep('E', 3)))
...
dat$exp <- '24G'                # CHANGE CHANGE CHANGE
...
write.xlsx(dat.table, file = '../ALFAM2 submission/dat_24G.xlsx')
