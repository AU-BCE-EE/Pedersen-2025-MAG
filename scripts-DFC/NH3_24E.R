...
dat$treat <- dat$id
dat$treat <- mapvalues(dat$treat, from = c('3', '7', '18'), to = c(rep('A', 3)))
dat$treat <- mapvalues(dat$treat, from = c('1', '10', '17'), to = c(rep('Cattle A', 3)))
dat$treat <- mapvalues(dat$treat, from = c('2', '8', '14'), to = c(rep('Pig A', 3)))
dat$treat <- mapvalues(dat$treat, from = c('6', '11', '13'), to = c(rep('H', 3)))
dat$treat <- mapvalues(dat$treat, from = c('4', '12', '15'), to = c(rep('D', 3)))
...
dat$exp <- '24E'                # CHANGE CHANGE CHANGE
...
write.xlsx(dat.table, file = '../ALFAM2 submission/dat_24E.xlsx')