...
```{r}
lm_a5 <- aov(e.rel.150 ~ TS.mn + pH.lab.mn + slurry.type, data = ds)
pairs(emmeans(lm_a5, ~ slurry.type), adjust = 'tukey')
predict(lm_a5, newdata = data.frame(TS.mn = mean(ds$TS.mn), pH.lab.mn = mean(ds$pH.lab.mn), slurry.type = c('Cattle', 'Pig', 'Digestate')))
```
...
```{r}
mm_d3 <- lmer(e.rel.150 ~ pH.lab.mn + n.mn + slurry.type + (1|trial.ID), data = ds)
summary(mm_d3)
