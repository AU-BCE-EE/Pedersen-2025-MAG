...
```{r}
mm_f1 <- lmer(e.rel.150 ~ slurry.type + (1|trial.ID), data = ds)
summary(mm_f1)
fixef(mm_f1)
...
```{r}
mm_f2 <- lmer(e.rel.150 ~ slurry.type + (1|trial.ID) - 1, data = ds)
summary(mm_f2)
fixef(mm_f2)
...
```{r}
lm_f1 <- lm(e.rel.150 ~ slurry.type + trial.ID, data = ds)
summary(lm_f1)
drop1(lm_f1, test = 'F')
```

```{r}
lm_f2 <- aov(e.rel.150 ~ slurry.type, data = ds)
summary(lm_f2)
model.tables(lm_f2, type = 'means')
TukeyHSD(lm_f2, 'slurry.type')
```
